...
public class AcceptNoDao {
    ...
    public boolean update(String studentID, String bookID, int amount, String cm) {
        ...
        try {
            ...
            if (st != null) {
                try {
                    st.close(); // chac chan de dong ket noi
                } catch (SQLException e) {
                    System.err.println("Failed to close statement: " + e.getMessage());
                    e.printStackTrace();
                }
    ...
    public ArrayList<AcceptNo> selectByName(String studentID) {
        ArrayList<AcceptNo> acceptList = new ArrayList<>();
        String query = "SELECT * FROM librarianacept WHERE studentID = ?";
        PreparedStatement st = null;
        ResultSet rs = null;
        ...
        try {
            ...
            while (rs.next()) {
                AcceptNo acceptNo = new AcceptNo();
                acceptNo.setBookID(rs.getString("MaSachID"));
                acceptNo.setAmount(Integer.parseInt(rs.getString("SoLuong")));
                acceptNo.setStudentID(rs.getString("studentID"));
                acceptNo.setStatus(rs.getString("ChoMuon"));
                acceptList.add(acceptNo);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {

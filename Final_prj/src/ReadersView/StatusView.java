...
public class StatusView extends JFrame {
    ...
    private void init() {
        ...
        btnTimkiem.setBounds(24, 195, 114, 23);
    ...
    public void ThucHienTimKiem() {
        AcceptNoDao acceptNoDao = new AcceptNoDao();
        String studentID = jtfMaSV.getText();
        XoaBang();
        if(!studentID.isEmpty()){
            ArrayList<AcceptNo> status = acceptNoDao.selectByName(studentID);
            if (status != null && !status.isEmpty()) {
                for (AcceptNo a : status) {
                    ThemVaoBang(a);
                }
            }
        }
    }
	public void ThemVaoBang(AcceptNo acceptNo){
        DefaultTableModel model_table = (DefaultTableModel) table.getModel();
        model_table.addRow(new Object[]{
            acceptNo.getStudentID(),
            acceptNo.getBookID(),
            acceptNo.getAmount(),
            acceptNo.getStatus()
        });
